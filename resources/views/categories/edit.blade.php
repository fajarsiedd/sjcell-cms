    ...
    <form action="{{ route('categories.update', $category) }}" method="POST">
        ...
        <div class="mb-5 w-full flex flex-row gap-10 mt-6">
            <div class="flex-1">
                ...
                <x-text-input type="text" id="name" name="name" value="{{ $category->name }}" required />
            </div>
            <div class="flex-1">
                <x-input-label for="name">Color <span class="text-red-600">*</span></x-input-label>
                <div class="flex flex-row" x-data="{ selectedColor: '{{ $category->color }}' }">
                    ...
                    <select id="color" name="color" x-on:change="selectedColor = $event.target.value"
                        ...
                        <option {{ $category->color == 'green' ? 'selected' : '' }} value="green">Green</option>
                        <option {{ $category->color == 'blue' ? 'selected' : '' }} value="blue">Blue</option>
                        <option {{ $category->color == 'yellow' ? 'selected' : '' }} value="yellow">Yellow</option>
                        <option {{ $category->color == 'purple' ? 'selected' : '' }} value="purple">Purple</option>
                        <option {{ $category->color == 'red' ? 'selected' : '' }} value="red">Red</option>
                    </select>
                </div>
            </div>
        </div>
